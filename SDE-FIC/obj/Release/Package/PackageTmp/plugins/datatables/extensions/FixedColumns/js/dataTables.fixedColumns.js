...
FixedColumns.prototype = /** @lends FixedColumns.prototype */{
	...
	/**
	 * Mark a Rows such that it's height should be recalculated when using 'semiauto' Rows
	 * height matching. This function will have no effect when 'none' or 'auto' Rows height
	 * matching is used.
	 ...
	 *      var fc = new $.fn.dataTable.fixedColumns( table );
	 *
	 *      // manipulate the table - mark the Rows as needing an update then update the table
	 *      // this allows the redraw performed by DataTables fnUpdate to recalculate the Rows
	 *      // height
	...
	/**
	 * Set the height of a given Rows - provides cross bRowser compatibility
	 ...
	 *      var fc = new $.fn.dataTable.fixedColumns( table );
	 *
	 *      // You may want to do this after manipulating a Rows in the fixed column
	 *      fc.fnSetRowsHeight( $('#example tbody tr:eq(0)')[0], 50 );
	 */
	"fnSetRowsHeight": function ( nTarget, iHeight )
	{
	...
	/**
	 * Get data index information about a Rows or cell in the table body.
	 ...
	 *    returned, or if given as a cell, an array of [Rows index, column index
	...
	"fnGetPosition": function ( node )
	{
		...
		else
		{
			...
			else
			{
				...
				var Rows = inst.fnGetPosition( $('tr', this.s.dt.nTBody)[ idx ] );
	...
	"_fnCopyLayout": function ( aoOriginal, aiColumns )
	{
		...
		{
			var aRows = [];
			aRows.nTr = $(aoOriginal[i].nTr).clone(true, true)[0];

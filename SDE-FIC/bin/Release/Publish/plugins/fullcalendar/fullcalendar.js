...
(function(factory) {
	...
	buttonIcons: {
		prev: 'left-single-arRows',
		next: 'right-single-arRows',
		prevYear: 'left-double-arRows',
		nextYear: 'right-double-arRows'
	},
...
var rtlDefaults = {
	...
	buttonIcons: {
		prev: 'right-single-arRows',
		next: 'left-single-arRows',
		prevYear: 'right-double-arRows',
		nextYear: 'left-double-arRows'
	},
	themeButtonIcons: {
...
// Given the scrollbar widths of some other container, create borders/margins on RowsEls in order to match the left
// and right space that was offset by the scrollbars. A 1-pixel border first, then margin beyond that.
function compensateScroll(RowsEls, scrollbarWidths) {
...
function uncompensateScroll(RowsEls) {
	RowsEls.css({
		'margin-left': '',
		'margin-right': '',
		'border-left-width': '',
...
// borRowsed from https://github.com/jquery/jquery-ui/blob/1.11.0/ui/core.js#L51
...
var GridCoordMap = Class.extend({
	...
	RowsCoords: null, // array of {top,bottom} objects
	...
	build: function() {
		this.RowsCoords = this.grid.computeRowsCoords();
	...
	getCell: function(x, y) {
		var RowsCoords = this.RowsCoords;
		...
		if (this.inBounds(x, y)) {
			...
			if (hitRows !== null && hitCol !== null) {
				cell = this.grid.getCell(hitRows, hitCol);
...
// They must have the same Rows, col, and be from the same grid.
...
----------------------------------------------------------------------------------------------------------------------*/
// It leverages methods of the subclass and the View to determine custom rendering behavior for each Rows "type"
// (such as highlight Rows, day Rows, helper Rows, etc).

var RowsRenderer = Class.extend({
	...
	// Renders the HTML for a Rows, leveraging custom cell-HTML-renderers based on the `RowsType`.
	// Also applies the "intro" and "outro" cells, which are specified by the subclass and views.
	// `Rows` is an optional Rows number.
	RowsHtml: function(RowsType, Rows) {
		var renderCell = this.getHtmlRenderer('cell', RowsType);
		var RowsCellHtml = '';
		var col;
		var cell;
		...
		for (col = 0; col < this.colCnt; col++) {
			cell = this.getCell(Rows, col);
			RowsCellHtml += renderCell(cell);
		}

		RowsCellHtml = this.bookendCells(RowsCellHtml, RowsType, Rows); // apply intro and outro

		return '<tr>' + RowsCellHtml + '</tr>';
	},
	...
	// `Rows` is an optional Rows number.
	bookendCells: function(cells, RowsType, Rows) {
		var intro = this.getHtmlRenderer('intro', RowsType)(Rows || 0);
		var outro = this.getHtmlRenderer('outro', RowsType)(Rows || 0);
	...
	// `RowsType` (like day, eventSkeleton, helperSkeleton), which is optional.
	// If a renderer for the specific RowsType doesn't exist, it will fall back to a generic renderer.
	// We will query the View object first for any custom rendering functions, then the methods of the subclass.
	getHtmlRenderer: function(rendererName, RowsType) {
		var view = this.view;
		var generalName; // like "cellHtml"
		var specificName; // like "dayCellHtml". based on RowsType
		var provider; // either the View or the RowsRenderer subclass, whichever provided the method
		var renderer;

		generalName = rendererName + 'Html';
		if (RowsType) {
			specificName = RowsType + capitaliseFirstLetter(rendererName) + 'Html';
		}
...
var Grid = fc.Grid = RowsRenderer.extend({

	start: null, // the date of the first cell
	end: null, // the date after the last cell

	RowsCnt: 0, // number of Rows
	colCnt: 0, // number of cols
	RowsData: null, // array of objects, holding misc data for each Rows
	...
	constructor: function() {
		RowsRenderer.apply(this, arguments); // call the super-constructor
	...
	// Responsible for setting RowsCnt/colCnt and any other Rows/col data
	...
	// Gets an object containing Rows/col number, misc data, and range information about the cell.
	// Accepts Rows/col values, an object with Rows/col properties, or a single-number offset from the first cell.
	getCell: function(Rows, col) {
		var cell;

		if (col == null) {
			if (typeof Rows === 'number') { // a single-number offset
				col = Rows % this.colCnt;
				Rows = Math.floor(Rows / this.colCnt);
			}
			else { // an object with Rows/col properties
				col = Rows.col;
				Rows = Rows.Rows;
			}
		}

		cell = { Rows: Rows, col: col };

		$.extend(cell, this.getRowsData(Rows), this.getColData(col));
	...
	// Retrieves misc data about the given Rows
	getRowsData: function(Rows) {
		return this.RowsData[Rows] || {};
	},
	...
	// Retrieves the element representing the given Rows
	getRowsEl: function(Rows) {
		// subclasses should implement if leveraging the default getCellDayEl() or computeRowsCoords()
	},
	...
	getCellDayEl: function(cell) {
		return this.getColEl(cell.col) || this.getRowsEl(cell.Rows);
	},
	...
	// By default, queries the dimensions of the element provided by getRowsEl().
	computeRowsCoords: function() {
		var items = [];
		var i, el;
		var item;
	...
	// Renders a day-of-week header Rows.
	// TODO: move to another class. not applicable to all Grids
	headHtml: function() {
		return '' +
			'<div class="fc-Rows ' + this.view.widgetHeaderClass + '">' +
				'<table>' +
					'<thead>' +
						this.RowsHtml('head') + // leverages RowsRenderer
					'</thead>' +
				'</table>' +
			'</div>';
	},
	...
	// Used by the `headHtml` method, via RowsRenderer, for rendering the HTML of a day-of-week header cell
	// TODO: move to another class. not applicable to all Grids
...
var DayGrid = Grid.extend({
	...
	numbersVisible: false, // should render a Rows for day/week numbers? set by outside view. TODO: make internal
	bottomCoordPadding: 0, // hack for extending the hit area for the last Rows of the coordinate grid
	breakOnWeeks: null, // should create a new Rows for each week? set by outside view

	cellDates: null, // flat chronological array of each cell's dates
	dayToCellOffsets: null, // maps days offsets from grid's start date, to cell offsets

	RowsEls: null, // set of fake Rows elements
	dayEls: null, // set of whole-day elements comprising the Rows's background
	helperEls: null, // set of cell skeleton elements for rendering the mock event "helper"


	// Renders the Rows and columns into the component's `this.el`, which should already be assigned.
	// isRigid determins whether the individual Rows should ignore the contents and be a constant height.
	// Relies on the view's colCnt and RowsCnt. In the future, this component should probably be self-sufficient.
	render: function(isRigid) {
		var view = this.view;
		var RowsCnt = this.RowsCnt;
		var colCnt = this.colCnt;
		var cellCnt = RowsCnt * colCnt;
		var html = '';
		var Rows;
		var i, cell;

		for (Rows = 0; Rows < RowsCnt; Rows++) {
			html += this.dayRowsHtml(Rows, isRigid);
		}
		this.el.html(html);

		this.RowsEls = this.el.find('.fc-Rows');
	...
	// Generates the HTML for a single Rows. `Rows` is the Rows number.
	dayRowsHtml: function(Rows, isRigid) {
		var view = this.view;
		var classes = [ 'fc-Rows', 'fc-week', view.widgetContentClass ];
		...
		return '' +
			'<div class="' + classes.join(' ') + '">' +
				'<div class="fc-bg">' +
					'<table>' +
						this.RowsHtml('day', Rows) + // leverages RowsRenderer. calls dayCellHtml()
					'</table>' +
				'</div>' +
				'<div class="fc-content-skeleton">' +
					'<table>' +
						(this.numbersVisible ?
							'<thead>' +
								this.RowsHtml('number', Rows) + // leverages RowsRenderer. View will define render method
							'</thead>' :
							''
							) +
					'</table>' +
	...
	// Renders the HTML for a whole-day cell. Will eventually end up in the day-Rows's background.
	// We go through a 'day' Rows type instead of just doing a 'bg' Rows type so that the View can do custom rendering
	...
	computeColHeadFormat: function() {
		if (this.RowsCnt > 1) { // more than one week Rows. day numbers will be in each cell
			return 'ddd'; // "Sat"
		}
	...
	// Initializes Rows/col information
	updateCells: function() {
		var cellDates;
		var firstDay;
		var RowsCnt;
		...
		if (this.breakOnWeeks) {
			...
			RowsCnt = Math.ceil(cellDates.length / colCnt);
		}
		else {
			RowsCnt = 1;
	...
	computeCellRange: function(cell) {
		var colCnt = this.colCnt;
		var index = cell.Rows * colCnt + (this.isRTL ? colCnt - cell.col - 1 : cell.col);
	...
	getCellDayEl: function(cell) {
		return this.dayEls.eq(cell.Rows * this.colCnt + cell.col);
	},


	// Overrides Grid's method for when Rows coordinates are computed
	computeRowsCoords: function() {
		var RowsCoords = Grid.prototype.computeRowsCoords.call(this); // call the super-method

		// hack for extending last Rows (used by AgendaView)
		RowsCoords[RowsCoords.length - 1].bottom += this.bottomCoordPadding;
	...
	// Slices up a date range by Rows into an array of segments
	rangeToSegs: function(range) {
		...
		var Rows;
		var RowsFirst, RowsLast; // inclusive cell-offset range for current Rows
		var isStart, isEnd;
		...
		for (Rows = 0; Rows < RowsCnt; Rows++) {
			RowsFirst = Rows * colCnt;
			RowsLast = RowsFirst + colCnt - 1;

			// intersect segment's offset range with the Rows's
			segFirst = Math.max(RowsFirst, first);
			segLast = Math.min(RowsLast, last);

			// deal with in-between indices
			segFirst = Math.ceil(segFirst); // in-between starts round to next cell
			segLast = Math.floor(segLast); // in-between ends round to prev cell

			if (segFirst <= segLast) { // was there any intersection with the current Rows?

				// must be matching integers to be the segment's start/end
				isStart = segFirst === first;
				isEnd = segLast === last;

				// translate offsets to be relative to start-of-Rows
				segFirst -= RowsFirst;
				segLast -= RowsFirst;

				seg = { Rows: Rows, isStart: isStart, isEnd: isEnd };
				if (isRTL) {
	...
	renderHelper: function(event, sourceSeg) {
		...
		// inject each new event skeleton into each associated Rows
		this.RowsEls.each(function(Rows, RowsNode) {
			var RowsEl = $(RowsNode); // the .fc-Rows
			var skeletonEl = $('<div class="fc-helper-skeleton"><table/></div>'); // will be absolutely positioned
			var skeletonTop;

			// If there is an original segment, match the top position. Otherwise, put it at the Rows's top level
			if (sourceSeg && sourceSeg.Rows === Rows) {
				skeletonTop = sourceSeg.el.position().top;
			}
			else {
				skeletonTop = RowsEl.find('.fc-content-skeleton tbody').position().top;
			}

			skeletonEl.css('top', skeletonTop)
				.find('table')
					.append(Rowstructs[Rows].tbodyEl);

			RowsEl.append(skeletonEl);
	...
	renderFill: function(type, segs) {
		...
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
			skeletonEl = this.renderFillRows(type, seg);
			this.RowsEls.eq(seg.Rows).append(skeletonEl);
	...
	// Generates the HTML needed for one Rows of a fill. Requires the seg's el to be rendered.
	renderFillRows: function(type, seg) {
		var colCnt = this.colCnt;
...
DayGrid.mixin({
	...
	Rowstructs: null, // an array of objects, each holding information about a Rows's foreground event-rendering
	...
	renderFgSegs: function(segs) {
		...
		// append to each Rows's content skeleton
		this.RowsEls.each(function(i, RowsNode) {
			$(RowsNode).find('.fc-content-skeleton > table').append(
	...
	// Uses the given events array to generate <tbody> elements that should be appended to each Rows's content skeleton.
	// Returns an array of Rowstruct objects (see the bottom of `renderSegRows`).
	...
	renderSegRows: function(segs) {
		...
		// iterate each Rows of segment groupings
		for (Rows = 0; Rows < segRows.length; Rows++) {
			Rowstructs.push(
				this.renderSegRows(Rows, segRows[Rows])
			);
		}
	...
	// Given a Rows # and an array of segments all in the same Rows, render a <tbody> element, a skeleton that contains
	// the segments. Returns object with a bunch of internal data about how the render was calculated.
	renderSegRows: function(Rows, Rowsegs) {
		var colCnt = this.colCnt;
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			// this single empty Rows is useful for styling.
			if (levelSegs) {
			...
			emptyCellsUntil(colCnt); // finish off the Rows
			this.bookendCells(tr, 'eventSkeleton');
			tbody.append(tr);
		}

		return { // a "Rowstruct"
			Rows: Rows, // the Rows number
			tbodyEl: tbody,
	...
	// Stacks a flat array of segments, which are all assumed to be in the same Rows, into subarrays of vertical levels.
	buildSegLevels: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// `j` now holds the desired subRows index
			seg.level = j;
	...
	// Given a flat array of segments, return an array of sub-arrays, grouped by each segment's Rows
	groupSegRows: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			segRows[segs[i].Rows].push(segs[i]);
		}
...
// Computes whether two segments' columns collide. They are assumed to be in the same Rows.
...
DayGrid.mixin({
	...
	// Limits the number of "levels" (vertically stacking layers of events) for each Rows of the grid.
	...
	limitRows: function(levelLimit) {
		...
		for (Rows = 0; Rows < Rowstructs.length; Rows++) {
			this.unlimitRows(Rows);

			if (!levelLimit) {
				RowsLevelLimit = false;
			}
			else if (typeof levelLimit === 'number') {
				RowsLevelLimit = levelLimit;
			}
			else {
				RowsLevelLimit = this.computeRowsLevelLimit(Rows);
			}

			if (RowsLevelLimit !== false) {
				this.limitRows(Rows, RowsLevelLimit);
			}
		}
	},


	// Computes the number of levels a Rows will accomodate without going outside its bounds.
	// Assumes the Rows is "rigid" (maintains a constant height regardless of what is inside).
	// `Rows` is the Rows number.
	computeRowsLevelLimit: function(Rows) {
		var RowsEl = this.RowsEls.eq(Rows); // the containing "fake" Rows div
		var RowsHeight = RowsEl.height(); // TODO: cache somehow?
		var trEls = this.Rowstructs[Rows].tbodyEl.children();
		var i, trEl;

		// Reveal one level <tr> at a time and stop when we find one out of bounds
		for (i = 0; i < trEls.length; i++) {
			trEl = trEls.eq(i).removeClass('fc-limited'); // get and reveal
			if (trEl.position().top + trEl.outerHeight() > RowsHeight) {
				return i;
			}
		}
	...
	// Limits the given grid Rows to the maximum number of levels and injects "more" links if necessary.
	// `Rows` is the Rows number.
	// `levelLimit` is a number for the maximum (inclusive) number of levels allowed.
	limitRows: function(Rows, levelLimit) {
		var _this = this;
		var Rowstruct = this.Rowstructs[Rows];
		var moreNodes = []; // array of "more" <a> links and <td> DOM nodes
		var col = 0; // col #, left-to-right (not chronologically)
		var cell;
		var levelSegs; // array of segment objects in the last allowable level, ordered left-to-right
		var cellMatrix; // a matrix (by level, then column) of all <td> jQuery elements in the Rows
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					...
					for (j = 0; j < colSegsBelow.length; j++) {
						...
						cell = this.getCell(Rows, seg.leftCol + j);
	...
	// Reveals all levels and removes all "more"-related elements for a grid's Rows.
	// `Rows` is a Rows number.
	unlimitRows: function(Rows) {
		var Rowstruct = this.Rowstructs[Rows];
	...
	showSegPopover: function(cell, moreLink, segs) {
		...
		if (this.RowsCnt == 1) {
			...
			topEl = this.RowsEls.eq(cell.Rows); // will align with top of Rows
		}
	...
	getCellSegs: function(cell, startLevel) {
		var segMatrix = this.Rowstructs[cell.Rows].segMatrix;
...
var TimeGrid = Grid.extend({
	...
	dayEls: null, // cells elements in the day-Rows background
	...
	renderHtml: function() {
		return '' +
			'<div class="fc-bg">' +
				'<table>' +
					this.RowsHtml('slotBg') + // leverages RowsRenderer, which will call slotBgCellHtml
				'</table>' +
			'</div>' +
			'<div class="fc-slats">' +
				'<table>' +
	...
	// This method is distinct from 'bg' because we wanted a new `RowsType` so the View could customize the rendering.
	...
	updateCells: function() {
		...
		this.RowsCnt = Math.ceil((this.maxTime - this.minTime) / this.snapDuration); // # of vertical snaps
	},


	// Given a cell object, generates a range object
	computeCellRange: function(cell) {
		var time = this.computeSnapTime(cell.Rows);
	...
	// Given a Rows number of the grid, representing a "snap", returns a time (Duration) from its start-of-day
	computeSnapTime: function(Rows) {
		return moment.duration(this.minTime + this.snapDuration * Rows);
	},
	...
	renderHelper: function(event, sourceSeg) {
		...
		// Try to make the segment that is in the same Rows as sourceSeg look the same
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
...
var BasicView = fcViews.basic = View.extend({
	...
	headRowsEl: null, // the fake Rows element of the day-of-week header
	...
	render: function() {
		...
		this.dayNumbersVisible = this.dayGrid.RowsCnt > 1; // TODO: make grid responsible
		this.weekNumbersVisible = this.opt('weekNumbers');
		this.dayGrid.numbersVisible = this.dayNumbersVisible || this.weekNumbersVisible;

		this.el.addClass('fc-basic-view').html(this.renderHtml());

		this.headRowsEl = this.el.find('thead .fc-Rows');
	...
	numberIntroHtml: function(Rows) {
		if (this.weekNumbersVisible) {
			return '' +
				'<td class="fc-week-number" ' + this.weekNumberStyleAttr() + '>' +
					'<span>' + // needed for matchCellWidths
						this.calendar.calculateWeekNumber(this.dayGrid.getCell(Rows, 0).start) +
					'</span>' +
				'</td>';
		}
	},
	...
	// Generates the HTML that goes before the day bg cells for each day-Rows.
	// Queried by the DayGrid subcomponent. Ordering depends on isRTL.
	...
	// Generates the HTML that goes before every other type of Rows generated by DayGrid. Ordering depends on isRTL.
	...
	// Generates the HTML for the <td>s of the "number" Rows in the DayGrid's content skeleton.
	// The number Rows will only exist if either day numbers or week numbers are turned on.
	...
	// Determines whether each Rows should have a constant height
	...
	setHeight: function(totalHeight, isAuto) {
		...
		uncompensateScroll(this.headRowsEl);
		...
		if (eventLimit && typeof eventLimit !== 'number') {
			this.dayGrid.limitRows(eventLimit); // limit the levels after the grid's Rows heights have been set
		}

		if (!isAuto && setPotentialScroller(this.scrollerEl, scrollerHeight)) { // using scrollbars?

			compensateScroll(this.headRowsEl, getScrollbarWidths(this.scrollerEl));
	...
	setGridHeight: function(height, isAuto) {
		if (isAuto) {
			undistributeHeight(this.dayGrid.RowsEls); // let the Rows be their natural height with no expanding
		}
		else {
			distributeHeight(this.dayGrid.RowsEls, height, true); // true = compensate for height-hogging Rows
		}
	},
	...
	renderEvents: function(events) {
		...
		this.updateHeight(); // must compensate for events that overflow the Rows
	},
...
var MonthView = fcViews.month = BasicView.extend({
	...
	setGridHeight: function(height, isAuto) {
		...
		// if auto, make the height of each Rows the height that it would be if there were 6 weeks
		if (isAuto) {
			height *= this.RowsCnt / 6;
		}

		distributeHeight(this.dayGrid.RowsEls, height, !isAuto); // if auto, don't compensate for height-hogging Rows
	},
...
fcViews.agenda = View.extend({ // AgendaView
	...
	noScrollRowsEls: null, // set of fake Rows elements that must compensate when scrollerEl has scrollbars
	...
	render: function() {
		...
		this.noScrollRowsEls = this.el.find('.fc-Rows:not(.fc-scroller *)'); // fake Rows not within the scroller
	},
	...
	setHeight: function(totalHeight, isAuto) {
		...
		unsetScroller(this.scrollerEl);
		uncompensateScroll(this.noScrollRowsEls);
		...
		if (!isAuto) { // should we force dimensions of the scroll container, or let the contents be natural height?
			...
			if (setPotentialScroller(this.scrollerEl, scrollerHeight)) { // using scrollbars?
				...
				compensateScroll(this.noScrollRowsEls, getScrollbarWidths(this.scrollerEl));
